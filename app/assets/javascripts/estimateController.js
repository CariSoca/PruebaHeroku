...
app.controller('MainController', function ($rootScope, $scope, $http) {
  ...
  //  $http.get("estimation.json").then(function(response) {                  
   //     $scope.projectName = response.data.projectName;                  
   //     $scope.serie = response.data.serie;                    
    //    $scope.strategy = response.data.consensus;
     //   $scope.timebox = response.data.timebox; 
     //   $scope.hour = $scope.timebox[0];
     //   $scope.min = $scope.timebox[1];
     //   $scope.number_of_vals = response.data.number_of_vals;
     //   $scope.values = Array.apply(null, {length: $scope.number_of_vals + 1}).map(Number.call, Number)
     //   $scope.values.shift();
    //    $scope.storiesData = response.data.storiesData;
   //     angular.forEach($scope.stories, function(story,index){
   //         var newStory = {id: story.id, name: story.name, puntaje: 0, total: 0}
